...
export function SessionSwitcher({ open, onClose }: Props) {
  ...
  const handleSelect = (terminalId: string, workspaceId: string) => {
    useWorkspacesStore.getState().setActiveProject(workspaceId);
    onClose();
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[var(--background)] border-t border-[var(--border)] max-h-[70vh] flex flex-col" style={{ paddingBottom: "calc(env(safe-area-inset-bottom) + var(--android-nav-lift, 0px))" }}>
              ...
              <div key={workspace} className="mt-3">
                ...
                <div className="mt-1 space-y-1">
                  {sessions.map((s) => (
                    <button
                      key={s.terminalId}
                      onClick={() => handleSelect(s.terminalId, s.workspaceId)}
                    ...
                    >
                      <div
                        ...
                        style={{ background: s.workspaceColor }}
                      />
